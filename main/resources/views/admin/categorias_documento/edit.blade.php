...
@section('header')
  <h1 class="text-2xl font-semibold">Editar Categoria de Documento</h1>
@endsection

@section('content')
  <form action="{{ route('admin.categorias_documento.update', $cat) }}" method="POST" class="space-y-4">
    @csrf
    @method('PUT')
    ...
    <div>
      ...
      <input type="text" name="nome" id="nome"
             value="{{ old('nome', $cat->nome) }}" required
    ...
    <div>
      ...
      <textarea name="descricao" id="descricao" rows="3" required
                class="w-full border-gray-300 rounded-md">{{ old('descricao', $cat->descricao) }}</textarea>
